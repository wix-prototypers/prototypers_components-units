...
function pcuComponentsEvents() {
  ...
  // dropdown focus
  document
    .querySelectorAll(".pcu-dropdown .pcu-text-input")
  ...
  document
    .querySelectorAll(".pcu-dropdown-item")
  ...
  document
    .querySelectorAll(
      '.pcu-dropdown[skin="badge"] .pcu-dropdown-content .pcu-dropdown-item'
    )
    .forEach(function (dropdownItem) {
      dropdownItem.addEventListener("click", function (e) {
        const input = e.target.parentElement.previousElementSibling;
  ...
  document
    .querySelectorAll(".pcu-float-notification .pcu-float-notification-close")
    .forEach(function (elm) {
      elm.addEventListener("click", function (e) {
  ...
  /* --------------------------- Popover Menu --------------------------- */
  document
    .querySelectorAll(".pcu-popover-menu")
    .forEach(function (popoverMenu) {
  ...
  /* --------------------------- Date Picker --------------------------- */
  document
    .querySelectorAll(".pcu-calendar-day")
    .forEach(function (dropdownItem) {
      dropdownItem.addEventListener("click", function (e) {
        ...
        if (!dayBtn.classList.contains("gray-day")) {
          ...
          dayBtn.closest(".pcu-date-picker").children[1].value =
            "09/" + theDay + "/2021";
        }
      });
    });
  ...
  /* --------------------------- Collapse Expand --------------------------- */
  document
    .querySelectorAll(".pcu-collapse-input")
    .forEach(function (collapseInput) {
      collapseInput.onchange = toggleCollpase;
    });

  function toggleCollpase() {
    let collapsible = this.closest(".pcu-collapse-container").querySelector(
      ".pcu-collapse-collapsible"
    );
...
PARAMETERS: name = for get the relevant input field, value = the selected value */
  function changesSliderWidth(sliderInput, value) {
    ...
    if (sliderStyleTag) {
      ...
      document.head.insertAdjacentHTML(
        "beforeend",
        `<style id='${styleTagID}'>${newStyling}</style>`
      );
    }
  ...
  document.querySelectorAll(".pcu-slider input").forEach((inputChanged) => {
    ...
    inputChanged.addEventListener("input", function (e) {
      ...
      if (inputElm.classList.contains("pcu-input-number")) {
      ...
      if (e.target.classList.contains("pcu-slider-input")) {
        // need to update the spinner value
        let spinnerField =
          inputChangedParent.querySelector(".pcu-input-number");
        spinnerField.value = selectedValue;
      }
  ...
  function changeOpacityColorPicker(sliderName, sliderValue) {
    ...
    let colorPicker = document.querySelector(
      `.pcu-slider-with-color .pcu-color-picker[slidername=${sliderName}]`
    );
